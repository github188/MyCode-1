/*
Binary Search. Connect the asc- and desc- array, and get the biggest middle (m+n)/2 elements. Attention to the special cases.
*/
class Solution {
public:
    ...
    double getAns(int A[],int m,int a1,int B[],int n,int b1,int x,int y)
    {
        if (x==0) 
        {
            ...
            {
                if ((y>1)&&((m==0)||(getNum(B,n,y-1,b1)>=getNum(A,m,m,a1)))) return (double)(getNum(B,n,y,b1)+getNum(B,n,y-1,b1))/2.0;
                else return (double)(getNum(B,n,y,b1)+getNum(A,m,m,a1))/2.0;
            }
        }
        if (y==0) 
        {
